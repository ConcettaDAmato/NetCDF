...
public class ReadNetCDFGEOSPACESoluteADEGrid1D {
	...
	@Description("Initial condition for solute concentration")
	@Out
	@Unit("M/m3")
	...
	@Description("Molecular Diffusion coeffincient")
	@Out
	@Unit("m2/s")
	public double[] molecularDiffusion;
	
	@Description("Longitudinal Dispersivity")
	@Out
	@Unit("m")
	...
	public void read() throws IOException {
		...
		if (step == 0) {
			...
			try {
				...
				Variable dataConcentrationIC = dataFile.findVariable("C0");
				...
				Variable dataMolecularDiffusion = dataFile.findVariable("molecularDiffusion");
				Variable dataLongitudinalDispersivity = dataFile.findVariable("longitudinalDispersivity");
				...
				concentrationIC = new double[size[0]];
				...
				ArrayDouble.D1 dataArrayConcentrationIC = (ArrayDouble.D1) dataConcentrationIC.read(null, size);
				...
				for (int i = 0; i < size[0]; i++) {
					...
					concentrationIC[i] = dataArrayConcentrationIC.get(i);
				...
				molecularDiffusion = new double[size[0]];
				longitudinalDispersivity = new double[size[0]];
				...
				ArrayDouble.D1 dataArrayMolecularDiffusion= (ArrayDouble.D1) dataMolecularDiffusion.read(null, size);
				ArrayDouble.D1 dataArrayLongitudinalDispersivity= (ArrayDouble.D1) dataLongitudinalDispersivity.read(null, size);
				...
				for (int i = 0; i < size[0]; i++) {
					...
					molecularDiffusion[i] = dataArrayMolecularDiffusion.get(i);
					longitudinalDispersivity[i] = dataArrayLongitudinalDispersivity.get(i);
